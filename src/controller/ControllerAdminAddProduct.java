	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if("load".equals(request.getParameter("type"))){
			...
			if(!discount.isEmpty() ){
				...
				objProduct = new Product(0, name, pictureNew, preview, detail, quatity, idPrice, idSale);
			}else{
				int idPrice = 0;
				int idSale = 0;
				Price objPrice = new Price(0, price);
			...
			if(mProduct.addItem(objProduct) > 0){
				response.sendRedirect(request.getContextPath() + "/admin/indexProduct?add=sucess");
				return ;
			}else{
